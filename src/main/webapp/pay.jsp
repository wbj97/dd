...
<body text=#000000 bgColor="#ffffff" leftMargin=0 topMargin=4>
	...
	<div id="main">
		...
		<form name=alipayment action=http://localhost:8989/alipay/alipay.trade.page.pay.jsp method=post
			target="_blank">
			<div id="body1" class="show" name="divcontent">
				<dl class="content">
					...
					<c:if test="${fn:length(shoppingCart)==1}">
						<dd>
							<input id="WIDsubject" name="WIDsubject" value="${c.value.bookName}"/>
						</dd>
					</c:if>
					
					<c:if test="${fn:length(shoppingCart)!=1}">
						<dd>
							<input id="WIDsubject" name="WIDsubject" value="图书"/>
						</dd>
					</c:if>
				
					<!-- 购买商品表单展示 -->
					<table class="tab_login" width="400" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td valign="top" class="w1" style="text-align: left">
								<b>&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;序号</b>
							</td>
							<td valign="top" class="w1" style="text-align: left">
								<b>商品名称</b>
							</td>
						...
						<c:forEach items="${sessionScope.cartMap}" var="map" varStatus="s">
		                <c:if test="${map.value.status==1}">
							...
							<tr>
								...
								<td valign="top">
									${map.value.book.bookName}
								</td>
								<td valign="top">
									${map.value.book.ddPrice}
								</td>
								<td valign="top">
									${map.value.count}
								</td>
					...
					<dt>付款金额 ：</dt>
					<dd>
						<input id="WIDtotal_amount" name="WIDtotal_amount" value="${sessionScope.totalPrice}" />
					</dd>
					<hr class="one_line">
					<dt>商品描述：</dt>
					<dd>
		...
		<form name=tradequery action=http://localhost:8989/testPay/alipay.trade.query.jsp method=post
			target="_blank">
		...
		<form name=traderefund action=http://localhost:8989/testPay/alipay.trade.refund.jsp method=post
			target="_blank">
		...
		<form name=traderefundquery
			action=http://localhost:8989/testPay/alipay.trade.fastpay.refund.query.jsp method=post
			target="_blank">
		...
		<form name=tradeclose action=http://localhost:8989/testPay/alipay.trade.close.jsp method=post
			target="_blank">
